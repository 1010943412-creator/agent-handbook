...
2. **关键机制（5 个要点）**
   ...
   - **与 CoT 的本质区别**：CoT 是纯推理（Thought 链），不接触外部世界；ReAct 是 Thought + Action + Observation，**Observation 是真值来源**——当模型推理出错时，工具返回的事实能纠正它。HotpotQA 上 ReAct 35.1% > CoT 28.7% > 仅 Action（无 Thought） 的 25.7%（数据来源：ReAct 论文 Table 1, 6-shot prompting, PaLM-540B）。
   - **失败模式**：① Thought 写得太啰嗦烧光 token（ReAct token 消耗是 CoT 的 3-5 倍，以 HotpotQA 2-shot 设定，ReAct 平均 token 数 vs CoT-Self-Consistency）；② Action 格式写错导致解析失败；③ Observation 信息密度低导致 Thought 卡住。三个坑在生产里都是必踩的。
...
```python
# 实战片段（需 API key：export ANTHROPIC_API_KEY=...）
