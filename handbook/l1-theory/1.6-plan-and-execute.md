...
2. **关键机制（5 个要点）**
   ...
   - **Re-Planner 的价值**：原始论文（Wang et al., 2023, *Plan-and-Execute Prompting*; arXiv:2305.14590；后续 LangChain Plan-and-Execute Agent 落地）显示，加 Re-Planner 后长任务成功率从 65% → 78%。Re-Planner 本身只需要重写剩余步骤，token 成本可控。
...
生产里 Plan-and-Execute 通常会进一步组合 **Reflexion**（1.7）——在每步执行后插入一个 Critic 节点评估质量，质量低就触发 Re-Planner。这是 LangChain Plan-and-Execute Agent 文档明确推荐的增强模式：
...
加入 Reflexion 后，单步失败能被立即发现并触发 Re-Planner，而不是拖到最后汇总答案时才发现——这能把长任务成功率再提 5-10 个百分点。
...
> 📚 本节参考
> - [S 级] Wang et al., 2023, *Plan-and-Execute Prompting* — https://arxiv.org/abs/2305.14590 （Plan-and-Execute 范式原始论文，Re-Planner 机制出处）
