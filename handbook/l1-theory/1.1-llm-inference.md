...
2. **关键机制（5 个要点）**
   ...
   - 显存账：`KV Cache ≈ 2 × n_layers × n_kv_heads × head_dim × seq_len × bytes_per_elem`，70B 模型在 128k 上下文下轻松吃掉 16–24GB 显存。
...
# 3) 估算 KV Cache 公式（70B 模型作为对照）
n_layers, n_kv_heads, head_dim = 80, 8, 128  # LLaMA-2-70B 配置
# LLaMA-2-70B: 80 layers, 8 KV heads (GQA), head_dim 128
# KV cache 大小: 2 * n_layers * seq_len * n_kv_heads * head_dim * 2 bytes (fp16)
# 128k context: ~10 GB
seq_len = 4096
bytes_per_elem = 2  # fp16
kv_gb = (2 * n_layers * n_kv_heads * head_dim * seq_len * bytes_per_elem) / 1024**3
print(f"[估算] LLaMA-2-70B 在 4k 上下文 KV Cache ≈ {kv_gb:.2f} GB")
# 实际跑出来：约 1.25 GB，和理论值一致
```
