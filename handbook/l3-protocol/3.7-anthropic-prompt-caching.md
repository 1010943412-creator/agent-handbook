...
> **本节钩子**：Anthropic Prompt Caching 在协议级把**"重复前缀"标记为可缓存**，命中后**输入 token 成本降 90%、延迟降 70%**（官方数据）——但**反直觉**的是：**缓存命中率 < 22% 时反而亏钱**，因为 `cache_write` 比普通输入贵 25%。这不是"开了就赚"，是"前缀稳定 + 调用密集才赚"。2.9 节讲过应用层 Semantic Cache，本节讲**协议层的 prefix cache**——两者互补。
...
2. **关键机制（5 个要点）**
   ...
   - **盈亏平衡点**：解方程 `(1-h) * 1.25 + h * 0.1 = 1`（即和不用缓存成本相同），得 `h ≈ 0.217`（**约 22%**）。也就是说命中率 < 22% 就亏钱，**生产里必须监控命中率**。
...
4. **常见误区**：
   ...
   - ❌ "开了缓存就省钱"——**错**。命中率 < 22% 反亏。**生产必须监控 cache_read_input_tokens / cache_creation_input_tokens**。
...
print(f"\n命中率: {hit_rate:.1%}")
# 生产里 hit_rate < 0.22 就亏钱（参考正文公式）
```

跑完你会看到——第一次请求 `cache_creation > 0`（写入缓存），第二次请求 `cache_read > 0`（命中缓存）。**重点是监控命中率**：< 22% 亏钱，> 22% 才赚。
...
2. **命中率 < 22% 亏钱**——生产必须监控 `cache_read / (cache_read + cache_creation)`；
