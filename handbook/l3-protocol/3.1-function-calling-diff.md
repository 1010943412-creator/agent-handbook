...
4. **常见误区**：
   ...
   - ❌ "Anthropic 没有 tool_choice 就不能强制"——**错**。**Anthropic 支持 `tool_choice: {type: 'tool', name: 'xxx'}` 强制指定工具**，但**不支持** `tool_choice='none'`（关闭）和 `tool_choice='required'`（必须调任一），与 OpenAI 的灵活度有差异。详见 [Anthropic Tool Use](https://docs.anthropic.com/en/docs/build-with-claude/tool-use/overview)。
