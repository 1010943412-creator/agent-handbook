...
最小 A2A server 示例：暴露 summarize_text 技能
依赖：a2a-sdk（pip install a2a-sdk，参考 https://github.com/a2aproject/a2a-python）
运行：python minimal_a2a_server.py
注意：a2a-sdk 处于快速迭代期，API 可能变动，以 https://github.com/a2aproject/a2a-samples 官方 sample 为准
"""
import uvicorn
...
summarize_skill = AgentSkill(
    ...
    tags=["summarization"],
    examples=["把这段长文浓缩成 3 句话"],
)

# 2) 定义 Agent Card（机器可读的简历）
...
# 3) 实现任务执行器（处理 tasks/send 进来的请求）
class SummarizeExecutor(AgentExecutor):
    """把用户文本浓缩成 3 句摘要（简化演示：实际项目调 LLM）"""
    ...
    async def execute(self, context: RequestContext, event_queue: EventQueue) -> None:
        ...
        await task_updater.update_status(TaskState.TASK_STATE_WORKING, new_text_message("正在摘要..."))

        # 3.2 拿用户文本（真实工程这里调 LLM）
        from a2a.helpers import get_message_text
        text = get_message_text(context.message) or ""
        summary = f"摘要(3 句): {text[:100]}..."

        # 3.3 写回 artifact + 标记完成
        await task_updater.add_artifact(parts=[new_text_part(text=summary, media_type="text/plain")])
        await task_updater.update_status(TaskState.TASK_STATE_COMPLETED, new_text_message("完成"))

    async def cancel(self, context: RequestContext, event_queue: EventQueue) -> None:
        raise NotImplementedError("cancel not supported")


# 4) 组装 handler + 路由 + ASGI app
...
routes.extend(create_jsonrpc_routes(request_handler, "/"))     # POST / (JSON-RPC 2.0 over HTTP)

app = Starlette(routes=routes)

if __name__ == "__main__":
    # A2A 协议允许 GRPC/HTTP_JSON/JSONRPC 多种 binding，生产里用 uvicorn 启动
...
# 研究 Agent 通过 A2A 协议委托代码 Agent 实现原型
import asyncio
import httpx
...
async def research_to_code_workflow():
    ...
    target_url = "http://code-agent.example.com"

    # 1) 研究 Agent 读代码 Agent 的 Agent Card
    async with httpx.AsyncClient() as httpx_client:
        resolver = A2ACardResolver(httpx_client=httpx_client, base_url=target_url)
        code_agent_card = await resolver.get_agent_card()

    # 2) 构造 client（真实 API: ClientConfig + create_client）
    config = ClientConfig(streaming=False)
    client = await create_client(agent=code_agent_card, client_config=config)

    # 3) 研究 Agent 不会写代码，构造消息委托给代码 Agent
    request = SendMessageRequest(
        message=new_text_message(
            "需要一个 Python 脚本，把 CSV 转成 Markdown 表格，include tests",
            role=Role.ROLE_USER,
        )
    )
    print("任务已发起，等代码 Agent 响应...")
    final_artifact = None
    async for chunk in client.send_message(request):
        # chunk 是 A2A 的 Task / Message / Artifact chunk，简化演示只取最后一条
        final_artifact = chunk

    # 4) 把代码保存到本地（生产用 webhook 推送而不是循环里等）
    if final_artifact is not None:
        print(f"代码 Agent 响应: {final_artifact}")
    await client.close()
...
下面是 **MCP + A2A 混用**的 server 示例——一个数据分析 Agent，**对外用 A2A 协议**（被其他 Agent 调），**对内用 MCP 协议**（调数据库）：
...
class DataAnalystExecutor(AgentExecutor):
    """A2A 调进来 -> 内部走 MCP 查 DB -> 回写 artifact。"""
    ...
    async def execute(self, context: RequestContext, event_queue: EventQueue) -> None:
        ...
        task_updater = TaskUpdater(event_queue=event_queue, task_id=task.id, context_id=task.context_id)
        await task_updater.update_status(TaskState.TASK_STATE_WORKING, new_text_message("查 DB 中..."))

        quarter = get_message_text(context.message) or ""

        # 2) 内部用 MCP 调数据库（stdio 模式连本地 postgres server）
        ...
        # 3) 实战片段：实际生成图表 + 上传 OSS（这里简化用 mock URL）
        chart_url = f"https://charts.example.com/q{quarter}.png"

        # 4) 写回 artifact + 标记完成
        await task_updater.add_artifact(parts=[new_text_part(text=f"{data}|chart={chart_url}", media_type="text/plain")])
        await task_updater.update_status(TaskState.TASK_STATE_COMPLETED, new_text_message("完成"))
