...
2. **关键机制（5 个要点）**
   ...
   - **Selective Context（Liu et al., 2023）**：基于自信息（self-information）筛选 token，每个 token 的"信息量" = -log P(token | context)。比 LLMLingua 简单（不需要小模型，单次 forward 算自信息），但准确率略低。
...
> - [S 级] Liu et al., 2023, *Selective Context* — https://arxiv.org/abs/2304.12182 （基于自信息的轻量压缩）
