...
注意：以下示例基于 Letta 0.6.x 的 `from letta import create_client` 老 API。
2025+ 版本 SDK 命名空间已迁移到 `letta_client`，安装方式与调用形态均有变化。
生产前请参考 https://docs.letta.com 最新文档与迁移指南。
"""
import requests
...
# 注意：以下示例基于 Letta 0.6.x 的 `from letta import create_client` 老 API。
# 2025+ 版本 SDK 命名空间已迁移到 `letta_client`，以下导入与调用需要按最新文档调整。
from letta import create_client
