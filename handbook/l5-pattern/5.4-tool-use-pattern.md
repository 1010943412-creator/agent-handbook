...
生产 Tool Use 通常配合"MCP 协议 + 工具注册表"——下面是 OpenAI Agents SDK 风格的最小实现：
...
实战要点：
- **工具注册表是核心**——把工具集中到 `TOOL_REGISTRY` 查表执行；比散落 if-else 好维护 10 倍。
- **JSON Schema 必填**——`description` 写清"做什么 + 返回什么"；`required` 显式声明必填参数。
- **MCP 优于自定义协议**——跨厂商 / 跨进程场景用 MCP（详见 3.3）比自建 HTTP/gRPC 好维护。
...
> - [S 级] L3.3 MCP 协议详解 — `handbook/l3-protocol/3.3-mcp-protocol.md`
> - [A 级] L3.1 Function Calling 协议 — `handbook/l3-protocol/3.1-function-calling-diff.md`
