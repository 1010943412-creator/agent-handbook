...
> **本节钩子**：Routing 不是"if-else 分发"——Supervisor 用 **LLM 动态决策**派给谁；**2025 新趋势**是 **Sub-agent Delegation**（Claude Agent SDK / LangGraph 强化）——父 Agent 把任务"完整委派"给子 Agent 而非"单步工具调用"。

## 正文大纲

1. **一句话定义**：Routing 是**中心化分发**——Supervisor 用 LLM 决策派给哪个子 Agent；子 Agent 独立完成子任务后返回。**关键观察**：Routing 与 5.7 Orchestrator-Workers 边界模糊——区别在于 Routing 的子任务"**结构已知**"（订单 / 账单 / 通用），Orchestrator-Workers 的子任务"**动态生成**"。
2. **适用场景**
   - **典型 1**：客服分流（订单 / 账单 / 物流 / 通用 Agent）—— 请求领域明确，Supervisor 一次决策。
   - **典型 2**：多领域专家（医疗 / 法律 / 编程 Agent）—— 子 Agent 专注一个领域。
   - **典型 3**：前端路由（FastAPI / Flask）派给不同 handler—— LLM 版"智能路由"。
...
3. **关键机制**（3 要点）
   - **Supervisor 用强 LLM**：GPT-4 / Opus 做 Supervisor（决策质量优先），子 Agent 可用小模型。
   - **Sub-agent 是完整委派**：子 Agent 有独立 system prompt + 工具集 + 上下文，完成后只返回"最终结果"。
   - **2025 Sub-agent Delegation 趋势**：Claude Agent SDK（sub-agents）+ LangGraph（Supervisor）+ OpenAI Agents SDK（handoffs）都原生支持"完整委派"。
4. **代码示例**：Routing 最小循环。
5. **常见误区**：
...
生产 Routing 配合"Sub-agent 隔离 + fallback"——下面是 LangGraph Supervisor 风格的最小实现：
...
general_agent = create_react_agent(
    ...
    prompt="你是通用助手,无法回答时 transfer_to_human",
)
...
workflow = create_supervisor(
    ...
    prompt=(
        "你是 Supervisor。派给最合适的子 Agent:\n"
        "- 订单 → order_agent\n"
        "- 账单 → billing_agent\n"
        "- 其他 → general_agent\n"
        "只返回子 Agent 结果,不要自己回答"
    ),
...
# ========== 3. 运行 ==========
# result = app.invoke({"messages": [{"role": "user", "content": "我的订单 #12345 还没发货"}]})
```

实战要点：
- **Sub-agent 隔离是关键**——每个子 Agent 独立工具集 + 独立 system prompt；避免"订单 Agent 也能调退款工具"的安全漏洞。
- **Supervisor 不接工具**——只做"决策 + 调度"；让"决策"和"执行"职责清晰。
- **fallback 必须有**——`general_agent` 处理"无法路由"的请求；不设置时 Supervisor 在边界场景会无限循环。
