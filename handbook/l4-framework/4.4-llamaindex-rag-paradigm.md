...
- **`FunctionAgent` 是 LLM 选工具 + 调工具的循环**——继承 `BaseWorkflowAgent`，底层走 Workflows 事件流（`AgentStream` / `AgentOutput`），比旧版 `FunctionCallingAgent` 更结构化（llama-index 0.14.x 重构后默认入口）。
