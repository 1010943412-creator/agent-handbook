...
1. **一句话定义**：Claude Agent SDK 是 Anthropic 在 2025-10（首个公开 release v0.1.3）开源的 Python SDK（基于 Claude Code 内部工具），定位为"长任务、工具深度集成、子 Agent 编排"；通过 `query()` 异步生成器获取消息，或 `ClaudeSDKClient` 多轮交互。
2. **关键机制（5 个要点）**
...
async def production_audit():
    ...
    options = ClaudeAgentOptions(
        ...
        max_turns=100,
        # 启用 1M 上下文窗口（context-1m-2025-08-07 beta）
...
- **长任务控制**：`max_turns=100` 防止 LLM 无限循环；长上下文任务可加 `betas=["context-1m-2025-08-07"]` 启用 1M 上下文窗口（见上方 `production_audit`）。
