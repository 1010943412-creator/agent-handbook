...
> **本节钩子**：从 2022-10 LangChain 0.1 早期 Python 包到 2025-09 Claude Agent SDK 公开，**3 年时间 Agent 框架走过 Web 框架 10 年的路**——从单 LLM 工具调用循环到多 Agent 状态机编排，从"全家桶"到 `langchain-core` 拆分，从单一第三方框架到**所有模型厂商都在自研官方 SDK**。**反直觉事实**：协议演进是"层叠"而非"替代"（3.9），**框架演进是"分化"而非"收敛"**——LangChain / LlamaIndex / AutoGen / CrewAI / OpenAI Agents SDK / Claude Agent SDK **6+ 框架共存**，往"独立生态"演化。
...
2. **关键机制（5 个要点）**
   ...
   - **2024-01 LangGraph 0.1**：LangChain 团队意识到"Agent 不是循环而是图"——发布 LangGraph，引入 `StateGraph` + 持久化 + HITL，**Agent 从循环抽象升级为状态机抽象**。
   - **2024-08 → 2024-12 协议层成熟**：AutoGen v0.4 重构（actor-model 异步运行时）+ LangChain 0.2 LCEL 稳定 + `langchain-core` 拆分——框架从"全家桶痛点"转为"分层架构"。
   - **2025 厂商官方 SDK 集中爆发**：OpenAI Agents SDK（2025-03）/ A2A v0.1（2025-04）/ CrewAI 1.0（2025-05）/ Claude Agent SDK（2025-09）——**半年内 4 个里程碑**，标志 Agent 框架从"社区驱动"转向"厂商主导"。
...
```mermaid
gantt
    title Agent 框架演进时间线 (2022-Q4 → 2025-Q3)
    dateFormat  YYYY-MM
    axisFormat  %Y-Q%q
    ...
    LangGraph 0.1 状态机            :lg1, 2024-01, 18M
    ...
    AutoGen v0.4 actor-model 重构   :ag2, 2024-08, 8M
    section CrewAI
    CrewAI 0.x 角色化协作           :cr1, 2024-01, 16M
    CrewAI 1.0 GA                  :cr2, 2025-05, 12M
    section 厂商官方 SDK
    OpenAI Agents SDK (Swarm 正式化) :oa1, 2025-03, 12M
    A2A 协议 v0.1                   :a2a, 2025-04, 12M
    Claude Agent SDK 公开           :ca1, 2025-09, 12M
```

- **辅助理解**：注意三个**抽象跃迁**节点——① **2023-01**（Chain → Agent 范式）、② **2024-01**（Agent → 状态机）、③ **2025-03**（第三方 → 官方 SDK）。**每年一次跃迁**，框架抽象在以"年"为单位进化，而 Web 框架以"代"为单位。
...
"""
Agent 框架演进里程碑表（2022-Q4 → 2025-Q3）。

数据来源：
- LangChain 官方 changelog 与版本博客
- LangGraph 发布公告
- AutoGen / CrewAI GitHub README 与 changelog
- OpenAI / Anthropic 官方公告
"""
...
milestones_phase2 = [
    Milestone("2024-01", "LangGraph", "0.1 状态机框架发布",
              "Agent 从循环抽象升级到状态机抽象，持久化 + HITL"),
    Milestone("2024-06", "LangChain", "0.2 + LCEL 稳定",
              "Runnable 接口统一所有可运行对象"),
    Milestone("2024-08", "AutoGen", "v0.4 actor-model 重构",
              "异步运行时 + 消息一等公民，API 不兼容 v0.2"),
    Milestone("2024-11", "Anthropic", "MCP 协议发布（外部依赖）",
              "Agent ↔ 工具通信标准，影响所有框架的适配方向"),
    Milestone("2024-12", "LangChain", "1.0 alpha + langchain-core 拆分",
              "解决'全家桶痛点'，分层架构成熟"),
]
...
milestones_phase3 = [
    ...
    Milestone("2025-04", "Google 牵头", "A2A 协议 v0.1",
              "Agent ↔ Agent 通信标准，50+ 公司联名"),
    Milestone("2025-05", "CrewAI", "1.0 GA",
              "角色化协作框架成熟，新增 Flows 事件驱动层"),
    Milestone("2025-09", "Anthropic", "Claude Agent SDK 公开",
...
# [2024-01] LangGraph: 0.1 状态机框架发布
# [2024-06] LangChain: 0.2 + LCEL 稳定
# [2024-08] AutoGen: v0.4 actor-model 重构
# [2024-11] Anthropic: MCP 协议发布（外部依赖）
# [2024-12] LangChain: 1.0 alpha + langchain-core 拆分
...
1. **概念辨析**：LangChain 1.0（2024-12 alpha → 2025-10 GA）"拆分 `langchain-core`"解决了 LangChain 0.x 哪个**具体痛点**？为什么这个问题在 0.x 时代没人解决，1.x 必须解决？
2. **场景判断**：2025-09 启动一个**生产级 Coding Agent**（小时级长任务、多文件编辑、子任务委派），下面哪个**最不合适**？为什么？
...
4. **间隔 1 个月**（2025-03 → 2025-04）。**同期重大事件**：CrewAI 1.0 GA（2025-05，间隔 2 个月）、LangChain 1.0 alpha → 正式 GA 推进期（2025-Q4）、MCP 协议 v1.0 稳定（2025-Q2）。**暗示**：① **模型厂商不再依赖第三方框架**——OpenAI / Anthropic / Google 都自研 SDK；② **协议层 OSS 化**（A2A + MCP）抵消厂商锁定风险；③ **第三方框架需要"中立性 + 跨厂商"作为护城河**，否则会被官方 SDK 蚕食；④ LangChain / LlamaIndex 的 `provider-agnostic` 定位反而成为优势。
