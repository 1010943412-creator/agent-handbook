...
4. **常见误区**：
   - ❌ "选 stars 最多的就对了"——LangChain 139.8k stars（截至 2026-06-21 GitHub API）最多但抽象层数最深，新手容易卡在 callback/LCEL 上；CrewAI 54k stars 但多 Agent 上手最快；AutoGen 59k stars（2025-Q3 起进入维护模式）。
...
graph TB
    ...
    subgraph 对话驱动多 Agent
        AG[AutoGen v0.4+<br/>维护模式]
        CR[CrewAI 1.x<br/>角色协作]
    end
    subgraph 厂商官方 SDK
...
3. **代码补全**：下面代码缺什么？补全让 CrewAI 真正运行：
   ...
   # 缺什么才能让 Crew.run() 工作？
   ```
4. **反直觉题**：有人说"AutoGen 5.9 万+ stars，所以新项目应该选它"。这个推断**正确吗**？为什么？请引用官方仓库的实际状态。
