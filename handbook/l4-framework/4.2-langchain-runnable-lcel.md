...
2. **关键机制（5 个要点）**
   ...
   - **LangChain 1.x 关键变化（2024-10 → 2025）**：`langchain-core` 拆分为独立 PyPI 包（`langchain-core` ≥ 0.3），第三方集成迁移到 `langchain-<provider>`（`langchain-openai` / `langchain-anthropic`）；`init_chat_model()` 统一入口；agent 创建从 `initialize_agent` 改为 `create_react_agent` / LangGraph。
