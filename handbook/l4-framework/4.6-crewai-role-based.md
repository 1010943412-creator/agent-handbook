...
1. **一句话定义**：CrewAI 是**角色化多 Agent 编排框架**——核心抽象是 `Agent`（角色）+ `Task`（任务）+ `Crew`（团队协作），执行模式分 `sequential` / `hierarchical`；2024-Q4 起新增 `Flows`（v0.74.0 = 2024-10-18 首发）提供事件驱动精细控制。
2. **关键机制（5 个要点）**
   ...
   - **Flows（2024-Q4 起,v0.74.0）**：精细事件驱动控制，用 `@start()` / `@listen(...)` / `@router(...)` 装饰器定义流程步骤；与 Crews 互补（Crews 负责"高层多 Agent 自治"，Flows 负责"精细状态机"）。
...
from crewai import LLM  # manager 用 LLM 决策（CrewAI 自带 LLM 类，与 LangChain 生态零依赖）
manager_llm = LLM(model="gpt-4.1")
