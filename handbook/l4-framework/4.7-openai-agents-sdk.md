...
2. **关键机制（5 个要点）**
   ...
   - **2025-09 后新增 Sandbox Agents**：在 v0.14.0 引入，预配置 computer environment + 文件系统 + Git repo，适合长任务（详见 L7 长任务章节）。
...
依赖：`openai-agents>=0.10`，演示单 Agent + tools + handoffs：

```python
"""
openai_agents_basic.py
OpenAI Agents SDK 最小示例
依赖：openai-agents>=0.10
