...
> **本节钩子**：Tracing ≠ 日志——日志是"事件流"，Trace 是"调用树"。**经验值**：超过 5 步的 Agent 调用链，单纯靠日志定位 bug 几乎不可能，必须用 Trace 才看得见。
...
2. **适用场景**（3 典型 + 2 反例）
   ...
   - **典型 2**：多 Agent 协作（5.5 Routing Supervisor / 5.7 Orchestrator-Workers）——多个子 Agent 各自产生 Trace，需在父 Trace 中按"树状"组织。
...
> Source: W3C Trace Context (https://www.w3.org/TR/trace-context/), OpenTelemetry Specification (2024).
